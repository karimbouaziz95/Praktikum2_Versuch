...
class LazyListe
{
   public:
	  ...
	  bool empty() const {if(p_ListeObjekte.empty()){return true;} else return false;}
     ...
     // Schreibfunktionen
      ...
      void vAktualisieren()
      {
         ...
         if ( bChanged ) //Kontrolle ob Aufgaben ausstehen
         {
	         
	         for (itL = p_ListeAktionen.begin(); itL != p_ListeAktionen.end(); ++itL)
	         {
			 (*itL)->vAusfuehren();   //Aktion durchführen
             delete (*itL);           //Aktion-Element löschen nach Durchführung
	         }
             p_ListeAktionen.clear(); //Liste leeren
	         bChanged = false;        //Fertigstelllung
         }
      }
