...
{
   public:
      LazyPushFront(const T& einObjekt, list<T>* ptListe)
...
{
public:
	LazyPushBack(const T& einObjekt, list<T>* ptListe)
		: LazyAktion<T>(ptListe)
		, p_tObjekt(einObjekt) {}
...
class LazyErase : public LazyAktion<T>
{
   ...
   public:
	   LazyErase(iterator& itObjekt, list<T>* ptListe) : LazyAktion<T>(ptListe), p_itObjekt(itObjekt){}
      virtual ~LazyErase() {}
