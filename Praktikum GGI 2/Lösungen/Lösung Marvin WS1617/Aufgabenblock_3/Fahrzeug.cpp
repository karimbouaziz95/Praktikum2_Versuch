...
Fahrzeug::Fahrzeug(const Fahrzeug& rhs)
	: AktivesVO(rhs)
{
	vInitialisierung(); // Zunächst alles mit null initialisieren
...
void Fahrzeug::vAusgabe() const
{
	...
	cout << setprecision(2) << fixed // 2 feste Nachkommastellen
		<< resetiosflags(ios::left) << setiosflags(ios::right) // rechtsbündig
...
double Fahrzeug::dGeschwindigkeit() const
{
	if (p_pVerhalten != nullptr)
	{
		...
		if (pWeg != nullptr)
		{
			double dLimit = pWeg->getMaxGeschwindigkeit();
