...
{
	vInitialisierung(); // Zunächst alles mit null initialisieren, neue ID vergeben
	p_sName = rhs.p_sName + " (K)";	// Namen ebenfalls übernehmen, aber als Kopie markieren
	p_dMaxGeschwindigkeit = rhs.p_dMaxGeschwindigkeit; // Max. Geschwindigkeit übernehmen
...
void Fahrzeug::vAusgabe() const
{
	cout << setprecision(2) << fixed // 2 feste Nachkommastellen
		<< resetiosflags(ios::left) << setiosflags(ios::right) << setw(2) << p_iID << " " // ID rechtsbündig
		<< resetiosflags(ios::right) << setiosflags(ios::left) << setw(10) << p_sName << ": " // Name linksbündig
		<< resetiosflags(ios::left) << setiosflags(ios::right) << setw(6) << p_dMaxGeschwindigkeit << " " // Max. Geschwindigkeit rechtsbündig
		<< setw(10) << dGeschwindigkeit() << " " // aktuelle Geschwindigkeit rechtsbündig
		<< setw(12) << p_dGesamtStrecke; // Gesamtstrecke rechtsbündig
}
