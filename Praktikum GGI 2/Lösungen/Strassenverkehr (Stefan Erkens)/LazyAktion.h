...
class LazyAktion
{
   ...
   protected:
      list<T>*	p_ptrLazyListe;	// Zeiger auf p_ListeObjekte
};
...
class LazyPushFront : public LazyAktion<T>
{
   public:
      ...
      void vAusfuehren() { p_ptrLazyListe->push_front(p_tObjekt); }
   private:
      T	p_tObjekt;
};
...
class LazyPushBack : public LazyAktion<T>
{
   public:
      ...
      void vAusfuehren() { p_ptrLazyListe->push_back(p_tObjekt); }
   private:
      T	p_tObjekt;
};
...
class LazyErase : public LazyAktion<T>
{
   ...
   public:
      LazyErase(iterator& itObjekt, list<T>* eineListe): LazyAktion<T>(eineListe) , p_itObjekt(itObjekt){}
      virtual ~LazyErase() {}
      void vAusfuehren() {p_ptrLazyListe->erase(p_itObjekt);}
   private:
